...
export class ChannelManagerImpl extends EventEmitter implements ChannelManager {
  ...
  private async buildChannel (sender: string, receiver: string, price: BigNumber.BigNumber, settlementPeriod: number, providedChannelId?: ChannelId | string): Promise<PaymentChannel> {
    const res = await this.channelContract.open(sender, receiver, price, settlementPeriod, providedChannelId)
